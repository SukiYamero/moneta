...
describe('GuestAdoptionPrompt', () => {
  ...
  it('accepting calls acceptGuestAdoption', async () => {
    ...
    render(<GuestAdoptionPrompt />)
    await userEvent.click(screen.getByRole('button', { name: /sí, traerlos/i }))
  ...
  it('shows a busy state and disables both actions while moving', () => {
    useAuthStore.setState({
      pendingAdoption: { profileId: 'p1', count: 3 },
      adoptionBusy: true,
    })
    render(<GuestAdoptionPrompt />)
    expect(screen.getByRole('button', { name: /moviendo/i })).toBeDisabled()

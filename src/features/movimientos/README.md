...
- `MovimientoFormFields.tsx` — the field set, presentational, driven entirely by the hook's return value: type toggle, date chip, amount, category field, then a note field behind a "ver más" disclosure. Composes `DateChipPicker`, `SegmentedControl`, `TextField`, `CategoryField`/`CategorySheet` (from `@/features/tags`) and `MovimientoAmountInput`. On a blocked submit, scrolls/focuses the field that blocked it.

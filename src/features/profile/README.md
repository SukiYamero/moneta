...
- `ProfileSheet.tsx` — the `BottomSheet` shell (`open`/`onClose` props), composing the sections below in order.
...
- `GuestAdoptionSection.tsx` + `useGuestAdoptionEntry.ts` — visible only when the active profile is Google-authenticated and has at least one guest movement not yet present in it (`src/lib/profiles/adoption.ts`'s `countUnadoptedGuestMovements`); the repeatable counterpart to the one-time login prompt (`src/features/auth/GuestAdoptionPrompt.tsx`). Tapping its CTA copies the delta via `adoptGuestMovements()` and shows an inline success confirmation.

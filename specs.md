...
- "Outside" is DOM containment (`wrapperRef.contains`), not raw coordinates. The pad's own box spans the full viewport width (`w-[100dvw] mx-[calc(50%-50dvw)]`, both sides) so the gutter beside the keys, past the sheet's own `px-5.5` side padding, belongs to the pad — the grid (`NumericKeypad`'s only rendered element for this caller) carries that same `px-5.5` itself, so the keys keep the sheet's own inset and render exactly where every other control in it does.

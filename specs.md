...
- The Profile sheet's own identity card follows the **active profile**, not the device's live Google session — `authStore` is device-scoped (§ Architecture and patterns) and stays authenticated in the background while a local/guest profile is switched to, so `IdentitySection` re-reads the active profile once `boot.ts`'s rebind settles and shows the guest card whenever it's `local`, even though `authStore.status` still reads `'authenticated'`.
...
**Implementation.** `src/lib/profiles/profileRegistry.ts` (explicit `activeProfileId` via `getActiveProfileId`/`setActiveProfileId`), `src/lib/profiles/profileOwner.ts` (owner marker), `src/lib/profiles/switchProfile.ts`, `src/features/profile/ProfilesSection.tsx`, `src/features/profile/IdentitySection.tsx`, `src/lib/dataStore.ts` (`runMutation`), `src/lib/outbox.ts`.
...
- **`IdentitySection`'s login CTA is the only way back to a Google profile from an active local one, even when the device already holds a live session for it** — tapping it re-runs the full OAuth consent flow (`authStore.login()`) instead of a lighter "switch back," which `ProfilesSection`'s own switcher already does with no popup. Not incorrect, just redundant friction.

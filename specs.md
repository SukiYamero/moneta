...
- "Outside" is DOM containment (`wrapperRef.contains`), not raw coordinates. **The pad's own box spans the full viewport width (`w-[100dvw] mx-[calc(50%-50dvw)]`, both sides) so the gutter beside the keys, past the sheet's own `px-5.5` side padding, belongs to the pad and can't dismiss it — but the grid carries that same `px-5.5` itself, so the keys keep the sheet's own inset and render exactly where every other control in it does.** Only the outer box bleeds; nothing duplicates the inset onto it, and nothing removes it from the grid. The grid's own `gap-3`, and the flex `gap-2` between the amount display and the pad, need no such treatment: a `gap` is spacing inside a container's box, not a hole in its hit-test area.

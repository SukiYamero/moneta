...
**Implementation.** `src/components/shared/useIsLandscape.ts` (`matchMedia` via `useSyncExternalStore`) + `LandscapeGuard.tsx` (presentation only, kept separate so a future design swaps `LandscapeGuard`'s body alone); `BottomNav.tsx`, `useOverlay.ts`, `useVisualViewportInset.ts` (overscan constants), `BottomSheet.tsx`/`CenterModal.tsx`.
